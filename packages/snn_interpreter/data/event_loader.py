...
Importing ``tonic`` is confined to :mod:`snn_interpreter.events.tonic_api`;
...
:class:`~snn_interpreter.events.event_sample.EventSample` by keeping ``x``
...
(``snn_interpreter.data.download_cli``): the child process constructs the
...
from snn_interpreter.data.datasets import dataset_spec
