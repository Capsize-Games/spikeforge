...
from snn_interpreter.energy import probe, target_costs
...
def account(
    ...
    ``source`` is a :class:`~snn_interpreter.event_runtime.SparseResult` or a
    raw counts mapping. A ``measurement`` (or one a device probe reports) makes
