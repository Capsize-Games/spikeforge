...
default (unconfigured) logging behaviour is unchanged. Enable JSON lines with
``SNN_LOG_JSON=1`` and a level with ``SNN_LOG_LEVEL=DEBUG``, then call
:func:`configure_logging`; :func:`reset_logging` removes the handler and
...
from snn_interpreter.observability.json_formatter import JsonFormatter

#: Package logger structured logging attaches to (never the root logger).
LOGGER_NAME = "snn_interpreter"
#: Default level when neither the argument nor ``SNN_LOG_LEVEL`` is set.
DEFAULT_LEVEL = "INFO"
...
def logging_enabled() -> bool:
    ...
    if _truthy(os.environ.get("SNN_LOG_JSON")):
        return True
    return bool(os.environ.get("SNN_LOG_LEVEL"))


def _level(name: Optional[str]) -> int:
    """Resolve a level name from the argument, environment, or default."""
    resolved = name or os.environ.get("SNN_LOG_LEVEL") or DEFAULT_LEVEL
...
def _use_json(json_mode: Optional[bool]) -> bool:
    ...
    if json_mode is not None:
        return json_mode
    return _truthy(os.environ.get("SNN_LOG_JSON"))
...
def configure_logging(
    ...
    ``level``/``json_mode`` override ``SNN_LOG_LEVEL``/``SNN_LOG_JSON``;
