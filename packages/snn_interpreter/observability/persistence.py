...
Persistence is opt-in: with ``SNN_METRICS_PERSIST`` unset the default instance
is disabled and :func:`flush` is a no-op that writes nothing, so the existing
...
from snn_interpreter.observability.store import SnapshotStore

#: Environment variable that opts a process into snapshot persistence.
ENV_FLAG = "SNN_METRICS_PERSIST"
...
def _env_enabled() -> bool:
    """Return True when ``SNN_METRICS_PERSIST`` is set to a truthy value."""
