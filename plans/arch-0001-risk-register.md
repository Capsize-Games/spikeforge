...
| PR-3 | **Packaging-authority migration.** `tests/test_packaging_profiles.py` reads `setup.py` text; introducing `pyproject.toml` silently drops that coverage. | Medium | Port the test to read `packages/*/pyproject.toml` in the same PR that adds the workspace; add a CI assertion that the set of extras and console scripts is unchanged by the migration. | Capsize Games |
