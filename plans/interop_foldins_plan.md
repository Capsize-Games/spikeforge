# SNN Interpreter — WS-F: Interop and Fold-In Gaps
...
|---|---|---|
| Event-dataset training | `build_dataset` refuses event specs (`spec.cls is None`) | [`datasets.py`](snn_interpreter/data/datasets.py:111) |
| Event loading | `EventSampleSource`, bridge, frames exist | [`event_source.py`](snn_interpreter/events/event_source.py), [`event_bridge.py`](snn_interpreter/events/event_bridge.py) |
| ONNX | Absent | [`setup.py`](setup.py:46) |
| `nirtorch` extraction | Probe exists, extraction deferred | [`api.py`](snn_interpreter/nir_bridge/api.py:63) |
| Quantization | Declared in constraints only | [`target_spec.py`](snn_interpreter/targets/target_spec.py:28) |
| Geometry | 28x28 / square conv math hardcoded | [`presets.py`](snn_interpreter/topology/presets.py:128), [`datasets.py`](snn_interpreter/data/datasets.py:73) |
...
- The image training path is unchanged; event training is additive behind the
  `events` extra ([`datasets.py`](snn_interpreter/data/datasets.py:51)).
- ONNX and extraction are opt-in extras; their absence is reported, never raised
  at import ([`probe.py`](snn_interpreter/targets/probe.py:1) pattern).
...
class ([`datasets.py`](snn_interpreter/data/datasets.py:111)), so training never
...
```
snn_interpreter/training/event_engine.py   EventTrainingEngine: steps over EventSampleSource
snn_interpreter/training/event_batches.py  batch an event stream into [T,B,...] via the bridge
```

- `event_batches` uses [`EventSpikeBridge`](snn_interpreter/events/event_bridge.py)
  to produce the same `[T, B, F]`/`[T, B, C, H, W]` contract the simulator
  already consumes, so the loss/optimizer loop reuses
  [`training_engine.py`](snn_interpreter/training/training_engine.py) unchanged.
...
- Honesty: when `tonic` is absent the engine raises the typed
  `EventsExtraMissingError` ([`event_errors.py`](snn_interpreter/data/event_errors.py)),
  and a synthetic event stream is never presented as a recording
  (see the `origin` convention in [`event_source.py`](snn_interpreter/events/event_source.py)).
...
- **Export** takes the built [`StageModule`](snn_interpreter/topology/stage_module.py)
...
**Gap.** [`api.py`](snn_interpreter/nir_bridge/api.py:63) probes `nirtorch` but
...
- Uses `nirtorch.extract_nir_graph` (isolated in
  [`api.py`](snn_interpreter/nir_bridge/api.py:1)) to lift an arbitrary
  `torch.nn.Module` into NIR.
- Any node `nirtorch` cannot map raises the existing typed
  `UnsupportedNodeError` in [`errors.py`](snn_interpreter/nir_bridge/errors.py),
  consistent with the ingest path ([`ingest.py`](snn_interpreter/nir_bridge/ingest.py:1)).
...
([`catalog.py`](snn_interpreter/targets/catalog.py:51)) but nothing applies it.
...
- Supported schemes (seed): `none` (no-op), `weight_int8`, `weight_uint8`
  ...
  [`catalog.py`](snn_interpreter/targets/catalog.py:86)).
...
- A quantization report lists per-layer before/after ranges and the drift this
  ...
  ([`drift.py`](snn_interpreter/nir_bridge/drift.py)).
...
The transform hardcodes 28x28 ([`datasets.py`](snn_interpreter/data/datasets.py:73))
and `conv_net` derives its feature size from a square side
([`presets.py`](snn_interpreter/topology/presets.py:128)).
