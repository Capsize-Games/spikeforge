...
The cache lives under :data:`snn_interpreter.config.HUB_CACHE_DIR` (env
``SNN_HUB_DIR``), alongside ``MODEL_DIR`` and outside the trained-model store,
so an imported foreign artifact never lands in the trained-model registry
until it is explicitly promoted.
"""

import os

from snn_interpreter.config import HUB_CACHE_DIR
