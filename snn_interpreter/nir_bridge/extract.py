...
:mod:`snn_interpreter.nir_bridge.api`, so the direct dependency stays in one
file. Modules ``nirtorch`` cannot map raise the typed
:class:`~snn_interpreter.nir_bridge.errors.UnsupportedNodeError` naming the
offending class; an unrecognised tracing operation raises the typed
:class:`~snn_interpreter.nir_bridge.errors.ExtractionError`. Nothing is ever
silently dropped. A successful graph is runnable by the independent
:class:`~snn_interpreter.nir_bridge.interpreter.NirInterpreter`, so a
...
from snn_interpreter.nir_bridge import api, torch_map
from snn_interpreter.nir_bridge.errors import (
    ExtractionError,
    ExtractionExtraMissingError,
    UnsupportedNodeError,
)
from snn_interpreter.nir_bridge.exporter import graph_summary
