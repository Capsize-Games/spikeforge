...
through :mod:`snn_interpreter.nir_bridge.api`. The envelope is version-stamped
...
from snn_interpreter.nir_bridge import api, array_codec
...
FORMAT_NAME = "snn-interpreter-nir-graph"
...
def _read_envelope(path: str) -> Dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("format") != FORMAT_NAME:
        raise MalformedGraphError(path, "not an snn-interpreter NIR graph")
